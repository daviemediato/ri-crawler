...
class PageFetcherTest(unittest.TestCase):
    ...
    def test_request_url(self):
        ...
        self.assertIsNotNone(
            strb_google,
            f'Não foi possível obter a URL: {obj_url_google.geturl()}')
        self.assertTrue(
            type(strb_google) == bytes,
            f'Ao requisitar uma URL válida, o tipo retornado deveria ser bytes'
        )
        self.assertIsNone(
            strb_jquery,
            'Ao ser requisitado alguma URL de um recurso que não é HTML, deve-se retornar None'
        )
    ...
    def test_discover_links(self):
        ...
        arr_expected_links = [
            (urlparse('https://www.pudim.com.br/lala.html'), 3),
        ...
        discovered_links = list(
            self.fetcher.discover_links(obj_url, 2, bin_str_content))
        self.assertEqual(
            len(arr_expected_links), len(discovered_links),
            f"A quantidade de links descobertos deveria ser {len(arr_expected_links)} e não {len(discovered_links)}"
        )

        for i, (url_link, depth) in enumerate(discovered_links):
            self.assertEqual(
                arr_expected_links[i][0].geturl(), url_link.geturl(),
                f"A {i + 1}ª URL extraída seria {arr_expected_links[i][0].geturl()} e não {url_link.geturl()}"
            )
            self.assertEqual(
                arr_expected_links[i][1], depth,
